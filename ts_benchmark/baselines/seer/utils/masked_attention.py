...
class Mahalanobis_mask(nn.Module):
    ...
    def calculate_prob_distance(self, X):
        ...
        dist = torch.einsum("bxcd,bxcd->bxc", temp, temp)  # 对应公式15吗
...
class CustomQueryAttention(nn.Module):
    def __init__(self, d_model, attn_mask=None, n_queries=3, num_var=7):
        super().__init__()
        self.queries = nn.Parameter(
            torch.randn(1, 1, n_queries, d_model)
        )  # [1, N, 1, D]
    ...
    def forward(self, enc_out, mask):
        batch, num_bar, d_model, num_patch = enc_out.shape
        n_queries = self.queries.size(0)

        kv = enc_out.permute(0, 1, 3, 2)

        # k = self.w_k(kv)
        # v = self.w_v(kv)  # [B, num_bar, num_patch, D]

        attn_scores = torch.matmul(self.queries, kv.transpose(-1, -2)) / (
                d_model ** 0.5
        )

        mask = mask.reshape(batch, num_bar, 1, -1)
        attn_scores = attn_scores * mask + (1 - mask) * (-math.log(1e10))

        attn_weights = F.softmax(attn_scores, dim=-1)  # 在num_patch维度做softmax
        # 加权求和: [B, n_queries, num_var, D]
        output = torch.matmul(attn_weights, kv)

        # 调整形状为 [B, num_var, D, n_queries]
